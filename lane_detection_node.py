...
class LaneDetection(Node):
    ...
    def locate_centroid(self, data):
        ...
        # TODO: Add code to detect mushroom
        mushroomLoc = detectMushroom(img)
        self.get_logger().info(f"Mushroom coordinates: " + mushroomLoc)
        ...
        try:
            ...
            # When Nothing was found
            	...
            	# make it turn right when no line is detected
                self.get_logger().info(f"Nothing detected")
                # self.centroid_error.data =0.3
                # self.centroid_error_publisher.publish(self.centroid_error)
...
def detectMushroom(inputImg):
    template = cv2.imread('mushroom.jpg', 0)
